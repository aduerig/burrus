...
def main():
    ...
    with tf.Session() as sess:
        ...
        x_data = np.array([np.arange(0, 128, dtype=np.float32), np.arange(0, 128, dtype=np.float32)])
        x_tensor = graph.get_tensor_by_name('x:0')
        value_head_dense_to_dense = graph.get_tensor_by_name('value_head_dense_to_dense/BiasAdd:0')
        ...
        res = sess.run([policy_head_output, value_head_output, value_head_dense_to_dense], feed_dict={x_tensor: x_data})
        print(res[2].shape)
